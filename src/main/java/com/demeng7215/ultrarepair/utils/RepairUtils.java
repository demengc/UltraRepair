...
public class RepairUtils {
	...
	public static void repairHand(Player p) {
		...
		if (p.getItemInHand().getType() == Material.AIR || p.getItemInHand().getType().isBlock() ||
		...
		if (!p.hasPermission("ultrarepair.bypass.cooldown") && System.currentTimeMillis() < timeout) {
			MessageUtils.tell(p, i.getMessages().getString("cooldown-active")
		...
		if (!p.hasPermission("ultrarepair.bypass.cooldown"))
			timeouts.put(p.getUniqueId(), System.currentTimeMillis() +
					(i.getSettings().getLong("repair-hand.cooldown") * 1000));
		...
		p.playSound(p.getLocation(), XSound.valueOf(i.getSettings().getString("repair-hand.sound")).parseSound(),
				100F, 100F);
	...
	public static void repairAll(Player p) {
		...
		for (ItemStack stack : p.getInventory().getContents()) {
			...
			if (stack != null && stack.getType() != Material.AIR &&
					!stack.getType().isBlock() && stack.getDurability() != 0) {
		...
		if (!p.hasPermission("ultrarepair.bypass.cooldown"))
			timeouts.put(p.getUniqueId(), System.currentTimeMillis() +
					(i.getSettings().getLong("repair-all.cooldown") * 1000));

		if (!p.hasPermission("ultrarepair.bypass.cost"))
			i.getEconomy().withdrawPlayer(p, cost);


		for (ItemStack stack : queue) stack.setDurability((short) 0);
		p.updateInventory();

		p.playSound(p.getLocation(), XSound.valueOf(i.getSettings().getString("repair-all.sound")).parseSound(),
				100F, 100F);
